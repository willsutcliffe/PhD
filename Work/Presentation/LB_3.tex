...
$|V_{ub}| = (3.23 \pm 0.31) \times 10^{-3}$
 ...
 % \item Uncertainty dominated by $ | f_{+} (q^{2})|^{2}$.
  ...
  \begin{center}
   \footnotesize{ \uline {Measured partial branching fraction [2]}:}
...
\begin{itemize}
  \item    \footnotesize  Uncertainty dominated by $ | f_{+} (q^{2})|^{2}$.  Lattice QCD predictions best at high $q^{2}$.
  \end{itemize}
  \vspace{0.8cm}
...
\hline
$P > 3000$ MeV/c & $P > 3000$ MeV/c & $cos \theta_{B_{s}(K\mu)} > 0.99$  \\
...
$\Delta LL(K-p) > 0$ & $\Delta LL(\mu-p) > 0$& $ \leq M_{K\mu} \leq 5500 MeV/c^{2}$  \\
...
\item Prescaled by a factor of 0.5.
\end{itemize}
 ...
 \begin{itemize}
  \setlength{\itemsep}{5pt}
    ...
    \item Signal Efficiency for stripping:   $ 7.2 \pm 0.1 \%$.
     \item Full efficiency in $4 \pi$, $\epsilon  \approx 1.4\%$.
    ...
    \item Belle: $21486 \pm 548$ signal events, Phys. Rev. D 83, 071101(R) (2011)
...
\end{itemize}
  ...
  \frame{
 \frametitle{Effect of Cuts on $q^{2}$ Distribution. }
 \begin{itemize}
  \setlength{\itemsep}{5pt}
   \item Remove following cuts from the stripping: $1500 MeV/c^{2} \leq M_{K\mu} \leq 5500 MeV/c^{2}$,  $cos \theta_{B_{s}Y} > 0.99$, $E_{\nu} < 2000 $MeV
   \item Plot shows the effect of individual cuts on the $q^{2}$ distribution.
...
%%%%%%%%%%%%
 ...
 \begin{itemize}
    \item Base selection on the current $B_{s} \rightarrow K \mu \nu$ line.
...
\end{tabular}
 ...
 \begin{itemize}
    \item Required retention $< 0.05 \%$ and timing $< 0.5$ ms/evt.
\end{itemize}
}
...
\hline
$P > 15000$ MeV/c & $P > 3000$ MeV/c & $cos \theta_{B_{s}(p\mu)} > 0.999$  \\
$p_{\text{T}} > 1000 MeV/c $        &$p_{\text{T}} > 1400 MeV/c$ & IP  $\chi^{2} > 16.0$   \\
Track $\chi^{2} <  6.0 $        & Track $\chi^{2} <  4.0 $ & Vertex $\chi^{2} < 3.0$  \\ 
Min IP $\chi^{2} > 16.0$ & Min IP $\chi^{2} > 12.0$ &  $\chi^{2}$ sep. from PV $> 100.0$\\
$\Delta LL(p-K) > 0$ & $\Delta LL(\mu-p) > 0$ & $2000 \leq M_{p\mu} \leq 5500 MeV/c^{2}$ \\
$\Delta LL(p-\pi) > 5 $ & $\Delta LL(\mu-\pi) > 3$& $p_{T(p\mu)} > 1500$MeV/c \\
$\Delta LL(p-\mu) > 0 $ & $\Delta LL(\mu-K) > 0$&
\end{tabular}

\begin{itemize}
 \setlength{\itemsep}{5pt}
\item StdLooseMuons and StdLooseProtons selections.
\item L0Muon \& (Hlt2SingleMuon | Hlt2TopoMu2Body)
\item Track Ghost probability $< 0.5$
\item Includes both signed combinations.
\end{itemize}
}

 \frame{
 \frametitle{ $\Lambda_{b} \rightarrow p \mu \nu$ Line: Tightened Selection }
\begin{tabular}{l*{3}{c}r}
$L_{b}\rightarrow p \mu \nu$ line             & Retention (\%) & Accepted & ms/evt  \\
\hline
Tightened Selection & 0.09 & 90 &  1.215   \\
\end{tabular}

  \begin{center}
 
    \includegraphics[width=0.35\textwidth]{Mpmu2.pdf}  
\includegraphics[width=0.35\textwidth]{Enu2.pdf} 

    \includegraphics[width=0.35\textwidth]{P_proton2.pdf}  
\end{center}
  \begin{center}
...
\section{Conclusion}
 ...
 \begin{itemize}
  \setlength{\itemsep}{5pt}
   \item $\Lambda_{b} \rightarrow p \mu \nu$ is a promising channel for an exclusive measurement of $V_{ub}$.
     \item Stripping line for $\Lambda_{b} \rightarrow p \mu \nu$ is almost ready.
   \end{itemize}
